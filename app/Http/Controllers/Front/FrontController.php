...
class FrontController extends Controller
{
    ...
    public function homePage() {
        ...
        $data['newProducts'] = Product::with(['image'])->where('status', 1)->limit(12)->orderBy('id','DESC')->get();
        ...
        ->limit(10)->get();
        $data['childCateBlogs'] = PostCategory::with(['posts' => function($q) {
            $q->where('status', 1);
    ...
    public function showProductDetail($slug) {
        try {
            ...
            return view('site.products.product_detail', compact('categories', 'product', 'productsRelated', 'category'));
    ...
    public function showProductList() {
        ...
        return view('site.products.product_category', compact('products', 'title'));
    }
    ...
    public function listBlog(Request $request, $slug)
    {
        ...
        if($category->getChilds()->count()) {
            $cate_ids = array_merge($cate_ids, $category->getChilds()->pluck('id')->toArray());
        }
    ...
    public function search(Request $request) {
        ...
        if (!empty($request->category)) {
            ...
            $query->whereIn('cate_id', $arr_category_id);
        }
        ...
        $categories = Category::parent()->with('products')->where(['type' => 1, 'parent_id' => 0])->orderBy('sort_order')->get();
        return view('site.products.product_category', compact('products', 'title', 'short_des', 'title_sub', 'categories'));
    }
}
